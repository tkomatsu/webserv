...
int Client::SetSocket(int _fd) {
  ...
  if (fd == -1) throw WebServ::Exception500("500");
  if (fcntl(fd, F_SETFL, O_NONBLOCK) != 0)
    throw WebServ::Exception500("500");
  return fd;
}
...
int Client::send(int client_fd) {
  ...
  if (ret > 0) {
    std::cout << "send to   " + host_ip_ << ":" << port_ << " " << response_.Str() << std::endl;
  }
  response_.Clear();
  request_.Clear();
  return ret;
...
void Client::SetPipe(int *pipe_write, int *pipe_read) {
  if (pipe(pipe_write) == -1) throw WebServ::Exception500("500");
  if (pipe(pipe_read) == -1) throw WebServ::Exception500("500");
}
...
void Client::CreateErrorResponse(const char * status_code) {
  if (std::strcmp(status_code, "500") == 0) {
    response_.SetStatusCode(500);
    // if error_pages:
        // set error_page
    // else:
        response_.SetBody("500");
  }
}
