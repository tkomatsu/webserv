...
void WebServ::start(void) {
  ...
  for (std::vector<Server>::iterator it = servers.begin(); it != servers.end();
    ...
    if (serv_fd > max_fd) max_fd = serv_fd;
  }
  ...
  while (true) {
    ...
    while (n == 0) {
      ...
      for (std::vector<Server>::iterator it = servers.begin();
           it != servers.end(); it++)
        for (std::vector<long>::iterator it_ =
                 (*it).writable_client_fds.begin();
    ...
    if (n > 0) {
      ...
      if (n > 0)
        for (std::vector<Server>::iterator it = servers.begin();
          ...
          for (std::vector<long>::iterator it = serv.client_fds.begin();
               it != serv.client_fds.end(); ++it) {
            long client_fd = *it;
